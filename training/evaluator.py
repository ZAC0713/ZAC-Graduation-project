...
def evaluate(prediction, ground_truth, mask, report=False):
    ...
    for i in range(prediction.shape[1]):
        ...
        """
        if i ==246:
            gt=np.argsort(ground_truth[:, i])[::-1]
            pre=np.argsort(prediction[:, i])[::-1]
            data1 = pd.DataFrame(gt)
            data1.to_csv("D:/Temporal_Relational_Stock_Ranking-master/data/result/2_NYSE gt&&pre btl and btl rank/2_NYSE valid/NYSE gt_btl.csv")
            data2 = pd.DataFrame(pre)
            data2.to_csv("D:/Temporal_Relational_Stock_Ranking-master/data/result/2_NYSE gt&&pre btl and btl rank/2_NYSE valid/NYSE pre_btl.csv")
        """
