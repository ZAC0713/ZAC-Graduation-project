...
class ReRaLSTM:
    def __init__(self, data_path, market_name, tickers_fname, relation_name,
                 emb_fname, parameters, steps=1, epochs=100, batch_size=None, flat=False, gpu=True, in_pro=False):
        ...
        # load data
        self.tickers = np.genfromtxt(os.path.join("D:/Temporal_Relational_Stock_Ranking-master/data/NYSE_tickers_qualify_dr-0.98_min-5_smooth.csv", '..', tickers_fname),
        ...
        self.rel_encoding, self.rel_mask = load_relation_data(
            os.path.join("D:/Temporal_Relational_Stock_Ranking-master/data", 'relation', self.relation_name,
        ...
        self.embedding = np.load(
            os.path.join("D:\Temporal_Relational_Stock_Ranking-master\data", 'pretrain', emb_fname))
    ...
    def train(self):
        ...
        with tf.device(device_name):
            ...
            """
            #每个元素减去最小值再除以最大值与最小值之差
            x1 = tf.reduce_max(return_ratio)
            y1 = tf.reduce_min(return_ratio)
            return_ratio_score=(return_ratio-y1)/(x1-y1)
            x2 = tf.reduce_max(ground_truth)
            y2 = tf.reduce_min(ground_truth)
            ground_truth_score=(ground_truth-y2)/(x2-y2)
            
            #每个元素除以序列中绝对值的最大值
            x = tf.reduce_max(tf.abs(return_ratio))
            return_ratio_score=tf.sigmoid(tf.scalar_mul(2.75/x,return_ratio))
            y=tf.reduce_max(tf.abs(ground_truth))
            ...
            ground_truth_score = tf.sigmoid(tf.scalar_mul(2.75 / y, ground_truth))
            """
            #单纯使用特征嵌入层的输出特征作为排位分数
            return_ratio_score=return_ratio
        ...
        for i in range(self.epochs):
            ...
            list[i][2] = tra_rank_loss / (self.valid_index - self.parameters['seq'] - self.steps + 1)
            ...
            list[i][5] = val_rank_loss / (self.test_index - self.valid_index)
            ...
            list[i][11] = test_rank_loss / (self.trade_dates - self.test_index)
        ...
        np.savetxt("D:/Temporal_Relational_Stock_Ranking-master/data/result/NYSE btl10/1.1_NYSE relation rank loss(mean square loss plus unchange sort loss-100 epoch) - 副本.csv", list, delimiter=',',header='Train Loss,Train reg loss,Train rank loss,Valid Loss,Valid reg loss,Valid rank loss,Valid_pr_mse,Valid_pr_mrrt,Valid_pr_btl,Test Loss,Test reg loss,Test rank loss,Test_pr_mse,Test_pr_mrrt,Test_pr_btl,Better valid loss')
